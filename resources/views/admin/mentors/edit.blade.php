<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <h2 class="text-xl font-bold text-gray-800">Edit Mentor — {{ $mentor->name }}</h2>
        </div>
    </x-slot>

    <div class="max-w-2xl">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <form method="POST" action="{{ route('admin.mentors.update', $mentor) }}" class="space-y-5">
                @csrf @method('PUT')
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    ...
                    <div>
                        ...
                        <input type="text" name="expertise" value="{{ old('expertise', $mentor->expertise) }}"
                        ...
                        <p class="mt-1 text-xs text-gray-400">Pisahkan dengan koma</p>
                ...
                <div>
                    ...
                    <textarea name="bio" rows="3"
                              class="w-full px-4 py-2.5 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">{{ old('bio', $mentor->bio) }}</textarea>
                </div>

                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1.5">Harga per Sesi (Rp) <span class="text-red-500">*</span></label>
                        <input type="number" name="price_per_session" value="{{ old('price_per_session', $mentor->price_per_session) }}" min="5000" step="5000"
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1.5">Durasi Sesi</label>
                        <select name="session_duration_minutes"
                                class="w-full px-4 py-2.5 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                            @foreach([30=>'30 menit',45=>'45 menit',60=>'60 menit',90=>'90 menit',120=>'120 menit'] as $val=>$label)
                                <option value="{{ $val }}" {{ old('session_duration_minutes', $mentor->session_duration_minutes) == $val ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="flex gap-3 pt-2">
                    <button type="submit" class="px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold rounded-xl transition-colors">Perbarui</button>
                    <a href="{{ route('admin.mentors.index') }}" class="px-6 py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm font-semibold rounded-xl transition-colors">Batal</a>
                </div>
            </form>
        </div>
    </div>
